...
#https://stackoverflow.com/questions/56064042/using-facet-tags-and-strip-labels-together-in-ggplot2
...
#fd_ses_df_lake_wider[!is.na(fd_ses_df_lake_wider$fdis_ses),]

fd_ses_df_lake_wider <- fd_combined %>% 
  pivot_wider(names_from = fd_metric, values_from = value) %>% 
  filter(!is.na(fdis_ses)) %>% 
...
fd_ses_df_lake_wider_remove_na <- fd_ses_df_lake_wider[!is.na(fd_ses_df_lake_wider$fdis_ses),]
...
colnames(fd_ses_df_lake_wider)

fd_ses_df_lake_wider[,c('fric_ses', "fdis_ses", "fdiv_ses")]
...
fish_mod_summary <- lapply(setNames(nm = names(alpha_fd_mods_mixed_fish)), function(x, mod_list) {
  ...
  return(
    data.frame(estimate = summary(mod_list[[x]])$coefficients$cond['fish1', 'Estimate'],
               z = summary(mod_list[[x]])$coefficients$cond['fish1', 'z value'],
...
multidim_fd_alpha_plots <- fd_longer_format %>%
  ...
  geom_point(aes(x = metric, y = value, group = fish, color = fish),
             size = 2, shape = 1, stroke = 1.5,
             position = position_jitterdodge(dodge.width = 0.875, jitter.width = 0.2)) +
  geom_boxplot(aes(x = metric, y = value, fill = fish),
               color = '#a6a6a6',
               alpha = 0.5, lwd = 0.8, outlier.shape = NA, #color = '#a6a6a6', 
  ...
  scale_fill_manual(values = c('#d16619', '#005AB5')) +
  scale_color_manual(values = c('#d16619', '#005AB5')) +
  theme_bw() +
...
ggsave(multidim_fd_alpha_plots,
       filename = here('figures', 'main', 'multidim_fd_alpha.png'), 
...
scheiner_alpha_plot <- fd_longer_format %>%
  ...
  geom_point(aes(x = metric, y = value, group = fish, color = fish),
             ...
             position = position_jitterdodge(dodge.width = 0.875, jitter.width = 0.2)) +
  geom_boxplot(aes(x = metric, y = value, fill = fish),
               alpha = 0.5, outlier.shape = NA, color = '#a6a6a6', lwd = 0.8, position = position_dodge(width = 0.875)) +
  #geom_violin(aes(x = metric, y = value, fill = fish),
  #            alpha = 0.3, color = '#a6a6a6', lwd = 0.8) +
  geom_text(data = fish_mod_summary %>%
              filter(metric %in% c('nsp', 'M.prime', 'qEt', 'qDTM')) %>% 
  ...
  theme(strip.text = element_text(size = 12),
        ...
        panel.grid.minor = element_blank(),
        panel.border = element_rect(colour = "gray", fill = NA, size = 1.25))


ggsave(tag_facet2(scheiner_alpha_plot, size = 6),
       filename = here('figures', 'main', 'scheiner_fd_alpha.png'), 
       width = 13*0.8, height = 7*0.8)
...
mixed_mod_variogram_multipan_list$FRic +
  theme(strip.text = element_text(size = 6))
mixed_mod_variogram_multipan_list$FDis
