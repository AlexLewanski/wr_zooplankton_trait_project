...
count_dat_processed_list <- readRDS(file = here('data', 
                                                'processed_data', 
                                                'count_dat_processed_list_daphnia_combine.rds'))
...
zoop_length_total_list0 <- readRDS(file = here('data', 
                                               'processed_data', 
                                               'zoop_length_total_list0_daphnia_combine.rds'))
...
all_taxa_traits <- readRDS(here('data', 
                                'processed_data', 
                                'all_taxa_trait_info_processed_daphnia_combine.rds'))


all_taxa_traits$mean_length[is.nan(all_taxa_traits$mean_length)] <- NA
all_taxa_traits$feeding_type[is.na(all_taxa_traits$feeding_type)] <- 'multiple'
...
#table(count_dat_processed_init$lake)
count_dat_processed_init <- count_dat_processed_list %>%
  ...
  filter(taxa_update %in% unique(rownames(all_taxa_traits_processed_init) )) %>%
...
all_taxa_traits_processed <- all_taxa_traits_processed_init %>% 
  filter(rownames(all_taxa_traits_processed_init) %in% unique(count_dat_processed_init$taxa_update))# %>% 
  #select(!taxa)
...
func_space_eval_fig <- rbind(zoo_msd_calc$func_eval$corank[,c('correction', 'dim', 'auc')] %>% 
        rename(value = auc) %>% 
        mutate(eval_method = 'AUC'),
      zoo_msd_calc$func_eval$mad %>% 
        rename(value = mad) %>% 
        mutate(eval_method = 'MAD')) %>% 
  ggplot() +
  geom_point(aes(x = dim, y = value, color = correction), size = 1.5) +
  geom_point(data = . %>% filter(dim == optimal_func_space_info$dim & correction == optimal_func_space_info$correction),
             aes(x = dim, y = value, color = correction),
             size = 3) +
  facet_wrap(~eval_method,
             nrow = 2, 
             strip.position = "left",
             scales = 'free_y') +
  ylab(NULL) +
  ...
  theme(strip.background = element_blank(),
        ...
        panel.border = element_rect(colour = "gray", fill=NA, linewidth = 1.25)) +
  xlab('Dimension') +
  scale_color_manual(values = c("#e9c46a", '#264653', "#2a9d8f", "#e76f51"))


ggsave(filename = here('figures', 'supplement', 'func_space_eval_fig.png'),
       plot = func_space_eval_fig,
       width = 6*1, height = 5.5*1)
...
count_dat_processed_full <- count_dat_processed_init %>%
  #filter(taxa %in% focal_taxa) %>%
  group_by(lake) %>% 
  #filter(n() >= (optimal_func_space_info$dim + 1) ) %>% 
  ungroup() %>%
...
ncol(count_dat_processed)/length(unique(count_dat_processed_init$lake))
...
set.seed(482938235)
randomized_com_list <- lapply(seq(1000), function(x, dat) {
  randomizeMatrix(t(dat %>% column_to_rownames('taxa_update')), 
                  null.model = c("frequency", "richness", "independentswap", "trialswap")[3], 
                  iterations = 1000) %>% 
    t() %>% 
    as.data.frame() %>% 
    rownames_to_column(var = 'taxa_update')
}, dat = count_dat_processed)
...
fd_ses_df_lake_plot <- fd_ses_df_lake %>%
  mutate(fish = if_else(fish == '1', 'fish', 'fishless'),
         metric = factor(dplyr::recode(fd_metric,
                                'fric_ses' = 'Richness',
                                'fdis_ses' = 'Dispersion',
                                'fdiv_ses' = 'Divergence',
                                'feve_ses' = 'Evenness'),
...
alpha_ftd <- FTD.comm(tdmat = zoo_dist, 
                      spmat = count_dat_processed_full %>% #count_dat_processed %>% 
                        column_to_rownames(var = 'taxa_update') %>% 
                        t(), 
                     q = 1, abund = TRUE, 
...
processed_scheiner_data_vis <- left_join(alpha_ftd, 
  ...
  mutate(fd_metric = factor(fd_metric, levels = c('nsp', 'M.prime', 'qEt', 'qDTM')),
         fd_metric = dplyr::recode(fd_metric, 'nsp' = 'Species richness') )
...
# 
# trait_list <- list(
#   repro_trophic_feeding_length = c('reproductive_mode', 'trophic_group', 'feeding_type', 'mean_length')
# )
# zoo_dist_list <- lapply(list(c()))
# 
# 
# zoo_dist <- dist_wrapper(df = all_taxa_traits_processed[,c('reproductive_mode', 'trophic_group', 'feeding_type', 'mean_length')],
#                          method = c('dist', 'gowdis', 'gawdis')[3], 
#                          arg_list = list(w.type = 'optimized', opti.maxiter = 500)) 
# 
# ### STEP 2: DETERMINE OPTIMAL FUNCTIONAL SPACE ###
...
# 
# 
# optimal_func_space_info <- zoo_msd_calc$func_eval$corank[which(zoo_msd_calc$func_eval$corank$dim <= 3),] %>% 
#   filter(auc == max(auc))
# 
# 
# 
# 
# rbind(zoo_msd_calc$func_eval$corank[,c('correction', 'dim', 'auc')] %>% 
#         rename(value = auc) %>% 
#         mutate(eval_method = 'AUC'),
#       zoo_msd_calc$func_eval$mad %>% 
#         rename(value = mad) %>% 
...
# 
# 
# optimal_func_space_info <- optimal_func_space_info[1,]
# 
# 
# 
# count_dat_processed <- count_dat_processed_init %>%
...
#   #filter(taxa %in% focal_taxa) %>%
#   group_by(lake) %>% 
#   filter(n() >= (optimal_func_space_info$dim) ) %>% 
#   ungroup() %>%
#   pivot_wider(names_from = lake, values_from = total_abund, values_fill = 0) %>% 
#   as.data.frame() %>% 
#   ncol()
...
# 
# 
# ggsave(filename = here('figures', 'updated_multidim_FD_violin_4trait.png'),
#        width = 13*0.8, height = 7*0.8)
# 
# 
# 
...
#   xlab('Metric') + ylab('Value')
# 
# ggsave(filename = here('figures', 'scheiner_fd_metrics_boxplot_4trait.png'), 
#        width = 13*0.8, height = 9*0.8)
...
# zoo_dist <- dist_wrapper(df = all_taxa_traits_processed[,c('trophic_group', 'feeding_type', 'mean_length')],
#                          method = c('dist', 'gowdis', 'gawdis')[3], 
#                          arg_list = list(w.type = 'optimized', opti.maxiter = 500)) 
# 
# ### STEP 2: DETERMINE OPTIMAL FUNCTIONAL SPACE ###
...
#                                         std_traits = TRUE,
#                                         pcoa_dims = optimal_func_space_info$dim,
#                                         correction = 'quasieuclid' , #optimal_func_space_info$correction,
#                                         feve_calc_type = c("reduced_PCoA", "FD")[1],
#                                         mst_package = c('vegan', 'ape')[1],
#                                         w_abund = TRUE,
#                                         calc_contr = FALSE,
...
# 
# 
# ggsave(filename = here('figures', 'updated_multidim_FD_violin_3trait.png'),
#        width = 13*0.8, height = 7*0.8)
# 
# 
# 
...
#   xlab('Metric') + ylab('Value')
# 
# ggsave(filename = here('figures', 'scheiner_fd_metrics_boxplot_3trait.png'), 
#        width = 13*0.8, height = 9*0.8)
