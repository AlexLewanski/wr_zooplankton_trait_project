...
length_processed <- subset_length_year %>%
  mutate(taxa_update = dplyr::recode(taxa_update, 
  ...
  filter(taxa_update != 'harpacticoida') %>%
  filter(Lake != "Lost Lake") %>% #remove lost lake
  mutate(lake = gsub(" ", "_", str_trim(tolower(Lake)))) %>%
  mutate(lake = dplyr::recode(lake,
                       'annika_lake' = 'annika',
  ...
  filter(taxa_update %in% focal_taxa) %>% 
  mutate(taxa_lake = paste0(taxa_update, "SPACE", lake))
...
count_dat_processed1 <- count_dat_processed_init %>%
  ...
  mutate(rel_abund = total_abund/sum(total_abund),
         taxa_lake = paste0(taxa_update, "SPACE", lake)) %>% 
  ungroup()
...
mannu_results <- lapply(setNames(nm = taxa_inboth_treatments), function(x, dat) {
  mannu_output <- wilcox.test(length_mm ~ fish, 
              data = dat %>% filter(taxa_update == x), 
              na.rm = TRUE, paired = FALSE, exact = FALSE, conf.int = TRUE)
  
  return(data.frame(taxa_update = x,
  ...
  bind_rows() %>% 
  mutate(factor = factor(taxa_update, levels = taxa_descending_order),
...
length_info_processed <- length_mean_info_fish %>% 
  ...
  mutate(fish_info = factor(if_else(fish == '0', 'Fishless', 'Fish'), levels = c('Fishless', 'Fish')),
         taxa_update = factor(taxa_update, levels = taxa_descending_order)) %>% 
  select(fish_info, taxa_update, fish_mean_length, overall_mean_length) %>% 
  ...
  pivot_longer(cols = !taxa_update, names_to = 'mean_type', values_to = 'value') %>% 
  as.data.frame()
...
species_level_shift_plot <- length_mean_info_fish_init %>%
  ...
  mutate(fish_info = factor(if_else(fish == '0', 'Fishless', 'Fish'), levels = c('Fishless', 'Fish')),
         taxa_update = factor(taxa_update, levels = taxa_descending_order)) %>% 
  ggplot(aes(y = taxa_update, x = length_mm, fill = fish_info, color = fish_info)) + #height = ..count..
  ...
  geom_jitter(data = . %>% 
              ...
              aes(y = as.numeric(taxa_update) - 0.03),
              size = 0.3, alpha = 0.7,
              position = position_jitterdodge(dodge.width = 0,
                                              jitter.width = 0.07,
                                              jitter.height = 0)) +
  geom_jitter(data = . %>% filter(fish_info == 'Fish'),
              aes(y = as.numeric(taxa_update) - 0.08),
              size = 0.3, alpha = 0.7,
              position = position_jitterdodge(dodge.width = 0,
                                              jitter.width = 0.07,
                                              jitter.height = 0)) +
  geom_point(data = length_info_processed %>% filter(mean_type == 'Fixed mean'),
             mapping = aes(y = as.numeric(taxa_update) + 0.02, x = value), size = 2.25, shape = 1, stroke = 1.4, color = '#495057', fill = '#495057') +
  geom_point(data = length_info_processed %>% filter(mean_type == 'Mean (fishless)' & !is.na(value)),
             mapping = aes(y = as.numeric(taxa_update) + 0.02, x = value), size = 2.25, shape = 1, stroke = 1.4, color = '#c1121f', fill = '#c1121f') +
  geom_point(data = length_info_processed %>% filter(mean_type == 'Mean (fish)' & !is.na(value)),
             mapping = aes(y = as.numeric(taxa_update) + 0.02, x = value), size = 2.25, shape = 1, stroke = 1.4, color = '#662e9b', fill = '#662e9b') +
  geom_textbox(
    data = mannu_results,
    aes(y = taxa_update, x = Inf, label = results_text),
...
count_dat_processed1_addzeros <- count_dat_processed1 %>% 
  select(taxa_update, lake, rel_abund) %>% 
  pivot_wider(values_from = rel_abund, names_from = 'taxa_update', values_fill = 0) %>% 
  pivot_longer(!lake, names_to = 'taxa_update', values_to = 'rel_abund')
