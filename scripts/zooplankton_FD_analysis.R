...
optimal_func_space_info <- zoo_msd_calc$func_eval$corank[which(zoo_msd_calc$func_eval$corank$dim <= 2),] %>% 
  filter(auc == max(auc))

#process community data based on optimal functional space
count_dat_processed <- count_dat_processed_list %>%
  bind_rows() %>% 
  group_by(Lake, taxa) %>%
...
if (identical(sort(count_dat_processed$taxa), sort(rownames(trait_info)))) {
...
#add in fish info to the ses dataframe and pivot to tidy format
fd_ses_df_lake <- left_join(fd_ses_df, fish_info[!duplicated(fish_info$lake),], by = 'lake')  %>%
  select(!community) %>% 
...
#' 
#' 
#' unique(count_processed_yearcombine[count_processed_yearcombine$Lake == 'Zigzag',]$taxa)
#' unique(zoop_length_total_list[zoop_length_total_list$Lake == 'Zigzag',]$taxa)
#' 
#' 
